...
public class MemberDAO {
	...
	public MemberVO isLogin(String id,String pwd)
	{
		...
		try
		{
			...
			{
				// 비밀번호 확인
				sql="SELECT id,pwd,name,sex FROM project_member WHERE id=?";
				ps=conn.prepareStatement(sql);
				ps.setString(1, id);
				rs=ps.executeQuery();
				rs.next();
	...
	public int memberIdCheck(String id)
	{
		...
		{
			getConnection();
			String sql="SELECT COUNT(*) FROM project_memeber WHERE id=?";
			ps=conn.prepareStatement(sql);
			ps.setString(1, id);
			ResultSet rs=ps.executeQuery();
			rs.next();
	...
	public int phoneIdCheck(String phone)
	{
		int count=0;
		try
		{
			getConnection();
			String sql="SELECT COUNT(*) FROM project_memeber WHERE phone=?";
			ps=conn.prepareStatement(sql);
	...
	// 5. 회원가입 => 회원 수종 => 회원 정보 출력 => 회원 탈퇴
	public void MemberJoin(MemberVO vo)
	{
		try
		{
			getConnection();
			String sql="INSERT INTO project_member VALUES(?,?,?,?,?,?,?,?,?,SYSDATE)";
			ps=conn.prepareStatement(sql);
